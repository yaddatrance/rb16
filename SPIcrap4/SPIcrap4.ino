...
static uint16_t frameBuffer[2][5000][5]; /* Framebuffer A*/  // avoid having to use volatile for shared variables by having two buffers
...
//  FlexiTimer2::start();
  ...
  myTimer.begin(kickpoint, 30);

  Serial.setTimeout(50); // max milliseconds when using readbytesuntil, parseint, etc
...
// then send points in "X Y X Y ... X Y"
  ...
  if (avail) {
    ...
    if ((c == '@') && (!nextFrameReady)) { // start of frame
      ...
      // got something weird, burn it with fire
      Serial.print("[");
